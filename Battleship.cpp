...
//{
//    auto& fleet = (board == playerBoard) ? playerFleet : computerFleet;
...
void Battleship::randomlyPlaceShips(std::vector<std::vector<char>>& board) {
    ...
    for (const auto& ship : fleet) {
        ...
        while (!placed) {
            ...
            if (horizontal && col + ship->getSize() <= BOARD_SIZE) {
                for (int j = col; j < col + ship->getSize(); ++j)
                    if (board[row][j] != '-') placed = false;
                if (placed) ship->placeShip(col, row, true);
            }
            else if (!horizontal && row + ship->getSize() <= BOARD_SIZE) {
                for (int j = row; j < row + ship->getSize(); ++j)
                    if (board[j][col] != '-') placed = false;
                if (placed) ship->placeShip(col, row, false);
            }
            else {
                placed = false;
            }
